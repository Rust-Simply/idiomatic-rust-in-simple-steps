...
tests", and this is true, but it's not a substitute for documentation. Easy to understand code is incredibly important
when you work in a code base so that you can easily understand it and maintain it. As we spoke about in the last
chapter, tests are important to show our expectations of the code. 
...
Up to now, we've been building binaries. Rust compiles binaries into executables, but those binaries can link to and,
...
this project right now (please don't though, this project is obviously useless to others 😅), then theoretically other
people could use your `add()` function. But, we aren't going to cover Crates at all in the language basics portion of
this book.

That doesn't mean this technique isn't useful though. Now that we've got our lib.rs, lets move `main.rs` to 
`bin/main.rs`. The program still runs if you use `cargo run` but if you look closely there is a difference.
...
library we've created. Feel free to try this out, but bear in mind that once you have more than one file in that
directory, you will have to tell cargo which binary you want it to focus on. For example
`cargo run --bin iriss-documentation`
...
Many languages like Java, JavaScript, PHP, and more, also provide a special type of comment called a Doc Comment. Like
...
we'll use them for most things like our add function.
...
That's neat, but we could have worked out that out from the name. Rust documentation uses markdown, which means we can
...
What's quite cool, is that if we go back to the top level of our library where our `add` function was listed, you can
...
That's right, Doc Tests. Some people say tests should be documentation, but here we say documentation can be the test!

Well, ok, not _the_ test, you should still write your unit tests. 😅
...
First, we seem to be getting three sets of tests. Second, we have passing tests... but we didn't write any tests, did
we?
...
Because the leading cause of bad documentation, and the reason why people so often don't bother writing it to start with
...
to the thing being documented makes it trivial to update.
...
get the source code which includes your documentation. This means when they build their documentation, your 
documentation becomes a part of it.
...
3. Code examples are run with tests, meaning it's very hard to make documentation that's wrong 

Furthermore, because the tooling is compelling enough to use on its own, everyone is already using it, meaning the
documentation for Rust projects has a 
